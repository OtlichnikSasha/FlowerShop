...
export const FavoritesProductsList: FC<FavoritesProductsListProps> = ({changeBasket, changeFavorite, firstLoading}) => {
    const {products, loading} = useTypedSelector(state => state.favorite)
    return (
        <div className="products_place">
            {
                products.length && !firstLoading ? products.map(product => {
                    ...
                    :
                    <Empty loading={loading} label="В избранном нет товаров!"/>
            }
        </div>
    );
};
